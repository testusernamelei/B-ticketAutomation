...
test('Draft_Coupon', async ({ page }) => {
  const login = new LoginPage(page);
  await login.goto();
  await login.login();
  const filePath = path.resolve(__dirname, '../assets/27e3e92a-a674-4206-81e2-658d56b64fc1'); //file path
  ...
  await expect(row.locator('td').nth(8)).toHaveText('Draft');
});